...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    emissionColor0 = hueShift(emissionColor0, frac(float(0.66) + float(1) * _Time.x) * float(1));
    #if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
        float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionMask) + _Time.x * float4(1,1,0,0)).r;
    #else
