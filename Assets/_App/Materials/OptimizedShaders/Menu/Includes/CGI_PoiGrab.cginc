    ...
    float4 blur(float2 uv)
    {
        ...
        float2 radius = float(0.5) / _ScreenParams.xy * 100; // Arbitrary constant to match old blur
        float quality = floor(float(10));
        float directions = floor(float(10));
    ...
    void calculateRefraction(float2 projectedGrabPos, inout float4 finalColor)
    {
        ...
        if(float(0) == 1)
        {
            refraction = Refraction(float(1.33), float(0.125), projectedGrabPos).rgb;
        }
        else
        {
            #ifdef CHROMATIC_ABERRATION_LOW
        ...
        float blendFactor = float(0.9) * POI2D_SAMPLER_PAN(_GrabPassBlendMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).r;
