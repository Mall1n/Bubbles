    ...
    void applyVoronoi(inout float4 finalColor, inout float3 VoronoiEmission)
    {
        ...
        randomPoint = 0;
        float4 voronoiColor1 = float4(0.01960665,0.01960665,0.01960665,1);
        float voronoi = 0;
        float3 position = 0;
        ...
        if(float(1) == 0) // Basic
        {
            voronoi = voronoi2D(position.xy, float(2), float4(1,1,1,1));
        }
        
        if (float(1) == 1) // Diff
        {
            voronoi = VoronoiNoiseDiff_Octaves(position, float(2), float4(1,1,1,1), voronoiOctaveNumber, voronoiOctaveScale, voronoiOctaveAttenuation, 1, _Time.x);
        }
        
        if (float(1) == 2) // Fixed Border
        {
            voronoi = voronoi3D(position, float(2), float4(1,1,1,1));
        }
        if (float(0) == 1)
        {
        ...
        float2 voronoiGradient = float4(0.03,0.03,0,0);
        voronoiGradient.xy += edgeNoise;
        float ramp = smoothstep(voronoiGradient.x, voronoiGradient.y, voronoi);
        
        if(float(0) == 0)
        {
            float4 voronoiColor = lerp(float4(0.2040006,0.2040006,0.2040006,1), voronoiColor1, ramp);
        ...
        float4 voronoiEmissionColor = lerp(float4(0.2040006,0.2040006,0.2040006,1) * float(0), voronoiColor1 * float(0), ramp);
