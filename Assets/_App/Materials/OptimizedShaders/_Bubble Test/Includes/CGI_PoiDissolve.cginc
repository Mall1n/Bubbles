...
void calculateDissolve(inout float4 albedo, inout float3 dissolveEmission)
{
    ...
    if (float(1) == 1) // Basic
    {
        ...
        edgeAlpha = remapClamped(noise, da + float(0.072), da, 0, 1) * (1 - dissolveAlpha);
    }
    ...
    if (float(0.072))
    {
        edgeColor = tex2D(_DissolveEdgeGradient, TRANSFORM_TEX(float2(edgeAlpha, edgeAlpha), _DissolveEdgeGradient)) * float4(1,1,1,0);
        #ifndef POI_SHADOW
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, edgeColor.rgb, smoothstep(0, 1 - float(0.186) * .99999999999, edgeAlpha));
    }
    dissolveEmission = lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(0), smoothstep(0, 1 - float(0.186) * .99999999999, edgeAlpha));
}
#endif
