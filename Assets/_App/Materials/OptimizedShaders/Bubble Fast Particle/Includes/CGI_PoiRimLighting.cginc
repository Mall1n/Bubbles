...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimStrength = float(3.48);
    float rimBrighten = float(0.45);
    float rimWidth = lerp( - .05, 1, float(0.402));
    #ifdef POI_AUDIOLINK
    ...
    rimWidth -= rimNoise;
    #if defined(PROP_RIMMASK) || !defined(OPTIMIZER_ENABLED)
        float rimMask = POI2D_SAMPLER_PAN(_RimMask, _MainTex, poiMesh.uv[float(0)], float4(1,1,0,0));
    #else
        float rimMask = 1;
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
